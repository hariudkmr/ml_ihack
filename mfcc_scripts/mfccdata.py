...
def setTemplateConfig(templateDirectory, cTemplateFilename, hTemplateFilename):
    global env, ctemplate, htemplate
    env = Environment(
        # For 3.0 version of jinja2, replace with
        loader=PackageLoader(templateDirectory,""),
        # loader=PackageLoader("mfccdata","mfcctemplates"),
        autoescape=select_autoescape(),
        trim_blocks=True
    )

    ctemplate = env.get_template(cTemplateFilename)
    htemplate = env.get_template(hTemplateFilename)
